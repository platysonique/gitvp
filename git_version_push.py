...
GITHUB_HEADERS = {"Accept": "application/vnd.github+json", "User-Agent": "github-version-push-tool/1.0"}
...
# ------------------------------------------
# Utility for async GitHub API (Tkinter safe)
def threaded(fn):
    def wrapper(*args, **kwargs):
        t = threading.Thread(target=lambda: fn(*args, **kwargs))
        t.daemon = True
        t.start()
    return wrapper
...
class GitHubDashboard(tk.Frame):
    ...
    def refresh(self):
        ...
        if not app.project_dir or not remote:
            ...
            return
        repo, owner = self.parse_owner_repo_from_remote(remote)
    ...
    def _display_commits(self, commits):
        ...
        for col in ['SHA', 'Author', 'Msg', 'Date']:
            ...
            tree.column(col, width=120 if col not in ("Msg",) else 260)
        for c in commits:
    ...
    def _display_prs(self, prs, owner, repo, headers):
        ...
        btns = {}
        btns['Approve'] = ttk.Button(btn_fr, text="Approve", command=lambda: self._pr_action(pr_selected(), 'APPROVE', owner, repo, headers, tree))
        btns['Approve'].pack(side='left', padx=3)
        btns['Merge'] = ttk.Button(btn_fr, text="Merge", command=lambda: self._merge_pr(pr_selected(), owner, repo, headers, tree))
        btns['Merge'].pack(side='left', padx=3)
        btns['Req. Changes'] = ttk.Button(btn_fr, text="Req. Changes", command=lambda: self._pr_action(pr_selected(), 'REQUEST_CHANGES', owner, repo, headers, tree))
        btns['Req. Changes'].pack(side='left', padx=3)
        btns['Comment'] = ttk.Button(btn_fr, text="Comment", command=lambda: self._comment_on_issue_or_pr(pr_selected(), owner, repo, headers, is_pr=True))
    ...
    def _show_pr_reviews(self, pr, owner, repo, headers):
        ...
        if r.status_code == 200:
            ...
            for review in reviews:
                out.append(f"{review['user']['login']}: {review['state']} ({review.get('body', '')})")
...
class GitApp(tk.Tk):
    ...
    def build_ui(self):
        ...
        self.notif_badge = ttk.Label(toolbar, text="", foreground='white', background='purple', padding=4, font=("TkDefaultFont", 9, 'bold'))
        ...
        row = ttk.Frame(f); row.pack(pady=6, fill='x', padx=10)
        ...
        options = ttk.Frame(f); options.pack(fill='x', padx=28, pady=4)
        ...
        sep = ttk.Separator(f, orient='horizontal'); sep.pack(fill='x', padx=24, pady=12)
        ttk.Label(f, text="Tags Management", font=('TkDefaultFont', 10, 'bold')).pack(anchor='w', padx=24)
        tagrow = ttk.Frame(f); tagrow.pack(anchor='w', padx=32, pady=(2,0))
        ...
        tagrow2 = ttk.Frame(f); tagrow2.pack(anchor='w', padx=32, pady=2)
        ...
        ttk.Button(sf, text="Save Credentials to Keyring", command=self.save_keyring_credentials).pack(pady=12)
        ttk.Button(sf, text="Clear Credentials from Keyring", command=self.clear_keyring_credentials).pack(pady=2)
        ttk.Label(sf, text="Credentials are stored securely using keyring.", wraplength=570, foreground="grey").pack(pady=4)
        ...
        # NEW: Button to create a desktop shortcut for this app
    ...
    def create_desktop_shortcut(self):
        ...
        icon_path = "/usr/share/icons/hicolor/96x96/apps/python3.png"  # You can supply your own icon if you like.
...
"""
        ...
        self.set_status(f"Shortcut created at {desktop_file}")
    ...
    def select_folder(self):
        ...
        if not candidates:
            self.set_status("No package.json found anywhere under that folder.")
            self.package_json_path = ""
            self.project_label.config(text='(None selected)')
            return
        ...
        try:
            ...
            self.set_status(f"Error reading package.json: {ex}")
            return
    ...
    def load_tags(self):
        ...
        try:
            ...
            self.set_status("Tag list refreshed.")
        except Exception as ex:
            self.set_status(f"Failed to get tags: {ex}")

    def create_tag(self):
        tag = self.new_tag_var.get().strip()
        if not tag:
            self.set_status("Enter new tag name."); return
        try:
            result = subprocess.run(["git", "tag", tag], cwd=self.project_dir, capture_output=True, text=True)
            if result.returncode == 0:
                self.set_status(f"Tag '{tag}' created locally.")
                self.load_tags()
            else:
                self.set_status(f"Failed to create tag: {result.stderr}")
        except Exception as ex:
            self.set_status(f"Error: {ex}")

    def delete_tag(self):
        tag = self.tag_var.get().strip()
        if not tag: self.set_status("Select a tag to delete."); return
        if not messagebox.askyesno("Delete tag?", f"Delete tag '{tag}' locally? This cannot be undone!"): return
        try:
            result = subprocess.run(["git", "tag", "-d", tag], cwd=self.project_dir, capture_output=True, text=True)
            if result.returncode == 0:
                self.set_status(f"Tag '{tag}' deleted locally.")
                self.load_tags()
            else:
                self.set_status(f"Failed to delete tag: {result.stderr}")
        except Exception as ex:
            self.set_status(f"Error: {ex}")

    def push_tag(self):
        tag = self.tag_var.get().strip()
        if not tag: self.set_status("Select a tag to push."); return
        try:
            result = subprocess.run(["git", "push", "origin", tag], cwd=self.project_dir, capture_output=True, text=True)
            if result.returncode == 0:
                self.set_status(f"Tag '{tag}' pushed to origin.")
            else:
                self.set_status(f"Failed to push tag: {result.stderr}")
        except Exception as ex:
            self.set_status(f"Error: {ex}")

    def get_branch_name(self, repo_dir=None):
        if repo_dir is None: repo_dir = self.project_dir
        try:
    ...
    def version_and_push(self):
        ...
        if not self.project_dir or not self.package_json_path:
            self.set_status("Select a project folder and package.json first.")
            return
        new_ver = self.new_version.get().strip()
        commit_msg = self.commit_msg.get().strip()
        if not new_ver or not commit_msg:
            self.set_status("You must enter both the new version and commit message.")
            return
        try:
            with open(self.package_json_path, 'r+') as f:
                ...
                self.set_status(f"Version updated: {old_ver} → {new_ver}")
        except Exception as ex:
            self.set_status("Failed to update package.json: " + str(ex)); return
        commands = [
        ...
        for cmd in commands:
            ...
            if status is not True:
                ...
                return
        self.set_status("Version updated, committed, tagged and pushed successfully!")
        self.load_tags()
        self.tab_dashboard.refresh()
        self.update_notif_badge()
    ...
    def save_keyring_credentials(self):
        ...
        if not keyring_available:
            ...
            return
        if user: keyring.set_password(SERVICE_NAME, 'github_user', user)
        if token: keyring.set_password(SERVICE_NAME, 'github_token', token)
        self.set_status("Credentials saved to system keyring for this tool.")

    def clear_keyring_credentials(self):
        if not keyring_available:
            messagebox.showerror("Keyring unavailable", "Python 'keyring' package is not installed.")
            return
        try: keyring.delete_password(SERVICE_NAME, 'github_user')
        except Exception: pass
        try: keyring.delete_password(SERVICE_NAME, 'github_token')
        except Exception: pass
        self.user_var.set(''); self.token_var.set('')
        self.set_status("Credentials erased from keyring.")
    ...
    def load_keyring_credentials(self):
        if keyring_available:
            try:
                ...
                if user: self.user_var.set(user)
                if token: self.token_var.set(token)
            except Exception: pass
